...
class SingleContextDecisionMaking(ngym.TrialEnv):
    ...
    def __init__(self, dt=100, context=0, rewards=None, timing=None,
        ...
        self.cohs = [5, 15, 50]
        self.cohs2 = [-5,-15,-50]
        self.cohs_all = [-5,-15,-50,5,15,50]
        self.cohs3 = [6,18,50]
        self.cohs4 = [-6,-18,-50]
        self.cohs5 = [-6,-18,-50, 6, 18,50]
    ...
    def _new_trial(self, **kwargs):
        ...
        trial['other_choice'] = 0 if trial['ground_truth'] == 1 else 1 

        trial['coh_0'] = random.choice(self.cohs)
        trial['coh_1'] = random.choice(self.cohs) 
        ...
        stim = np.cos(self.theta - stim_theta_0) * (signed_coh_0 / 200) + 0.5
        self.add_ob(stim, 'stimulus', where='stimulus_mod1')

        stim = np.cos(self.theta - stim_theta_1) * (signed_coh_1 / 200) + 0.5
...
class ASS_SD(ngym.TrialEnv):
    
    def __init__(self, dt=100, context=0, rewards=None, timing=None,
                 sigma=1.0, dim_ring=1):
        super().__init__(dt=dt)
        ...
        # Sound Stim Variables
        self.sRate = 10000 # sample rate in Hertz
        self.freq1 = 5.0 # frequency of first sine wave
        self.freq2 = 50.0 # frequency of second sine wave
        self.amplitude1 = 10 # amplitude of first wave
        self.amplitude2 = 5 # amplitude of second wave
        self.numPeriods = 2 # number of periods of the sine waves
        self.numSamples = self.sRate * self.numPeriods # total number of samples
        ...
        # set action and observation space
        self.theta = np.linspace(0, self.numPeriods, self.numSamples)
        self.choices = np.arange(1)

        name = {
            'fixation': 0,
            'stimulus_mod1': range(1, 2),
        }
        shape = (1 + 2 * dim_ring,)
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=shape, dtype=np.float32, name=name)
    ...
    def _new_trial(self, **kwargs):
        ...
        trial['other_choice'] = 0 if trial['ground_truth'] == 1 else 1 
        trial['coh_0'] = 50 if trial['ground_truth'] == 1 else 5
        #trial['coh_1'] = random.choice(self.cohs3)
        #elif trial['context'] == 1:
         #   trial['coh_0'] = random.choice(self.cohs_all)
          #  trial['coh_1'] = random.choice(self.cohs3) if trial['ground_truth'] == 1 else random.choice(self.cohs4)
        ...
        #self.add_ob(1, where='fixation')
        f1 = lambda x: self.amplitude1*np.sin(coh_0*2*np.pi*self.theta)
        stim = self.sampled_f1 = [f1(i) for i in self.theta]
        self.add_ob(stim, 'stimulus', where='stimulus_mod1')
        self.add_randn(0, self.sigma, 'stimulus')
        self.set_ob(0, 'decision')
